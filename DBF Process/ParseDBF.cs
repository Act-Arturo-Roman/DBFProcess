            ...
            foreach (FieldDescriptor field in fields)
            {
                ...
                switch (field.fieldType)
                {
					case 'N':
						if (number.IndexOf(".") > -1)
						{
                            col = new DataColumn(field.fieldName + "*" + field.fieldLen.ToString(), typeof(decimal));
						}
						else
						{
                            col = new DataColumn(field.fieldName + "*" + field.fieldLen.ToString(), typeof(int));
						}
						break;
					case 'C':
                        col = new DataColumn(field.fieldName + "*" + field.fieldLen.ToString(), typeof(string));
						break;
					case 'T':
						// You can uncomment this to see the time component in the grid
						//col = new DataColumn(field.fieldName, typeof(string));
                        col = new DataColumn(field.fieldName + "*" + field.fieldLen.ToString(), typeof(DateTime));
						break;
					case 'D':
                        col = new DataColumn(field.fieldName + "*" + field.fieldLen.ToString(), typeof(DateTime));
						break;
					case 'L':
                        col = new DataColumn(field.fieldName + "*" + field.fieldLen.ToString(), typeof(bool));
						break;
					case 'F':
                        col = new DataColumn(field.fieldName + "*" + field.fieldLen.ToString(), typeof(Double));
						break;
                }
                dt.Columns.Add(col);
            }
            ...
            for (int counter = 0; counter <= header.numRecords - 1; counter++)
            {
                ...
                foreach (FieldDescriptor field in fields)
                {
                    switch (field.fieldType)
                    {
						...
						case 'C': // String
                            row[fieldIndex] = Encoding.GetEncoding("ISO-8859-1").GetString(recReader.ReadBytes(field.fieldLen));
                            break;
